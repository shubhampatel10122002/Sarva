...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-primary/70 backdrop-blur-2xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        ...
        <div className="hidden items-center gap-8 md:flex">
          ...
          <a
            href="#contact"
            className="rounded-lg bg-accent px-5 py-2 text-[13px] font-medium text-white transition-all hover:bg-accent-light"
          >
            Get in Touch
          </a>
        </div>
        ...
        <div className="border-t border-white/5 bg-primary/95 px-6 py-6 backdrop-blur-2xl md:hidden">
          {navLinks.map((link) => (
            <a
              ...
              className="block py-3 text-sm text-muted-light transition-colors hover:text-secondary"
            >
              {link.label}
            </a>
          ))}
